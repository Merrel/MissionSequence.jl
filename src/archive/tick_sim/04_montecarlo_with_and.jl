...
function run(start_β_multiplier)

    # Update the start_β duration stochasitally
    # start_β.duration = round(rand(Exponential(0.5)) * start_β_multiplier)
    ...
    start_β = Event(
        ...
        1.0,
        Dict(:pass => "launch_β", :fail => :LOM),
        round(rand(Exponential(0.5)) * start_β_multiplier)
    )
...
#     prog = ProgressThresh(ϵ, "Running to convergence:")
...
#         if n > 100
#             ProgressMeter.update!(prog, ΔR)
#             # println(ΔR)
#             if ΔR < ϵ
#                 println("\nConverged in $n simulations --> reliability = $R")
#                 break
#             end
#         end
#     end
